...
# Extraer sitios de lanzamiento únicos
...
def update_scatter_chart(entered_site, payload_range):
    # Filtrar datos según el rango de carga útil
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= payload_range[0]) & 
                             (spacex_df['Payload Mass (kg)'] <= payload_range[1])]
    
    if entered_site == 'ALL':
        # Si se seleccionan todos los sitios, mostrar todas las cargas útiles y éxitos
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', 
                         color='Booster Version Category',
                         title='Correlation between Payload and Success for All Sites')
    else:
        # Filtrar por el sitio de lanzamiento seleccionado
